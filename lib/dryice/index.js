...
copy.filter.moduleDefines = function(input, source) {
  ...
  function replacer(match, moduleArgs) {
    var moduleDeps;
    if (moduleArgs === 'require') {
      moduleDeps = ['require', 'exports', 'module'].concat(deps);
    } else {
      moduleDeps = ['require'].concat(deps);
    }
    return "define('" + module + "', [" + moduleDeps.join("', '") + "], " +
           "function(" + moduleArgs + ") {";
  }

  return input.replace(
    /\bdefine\s*\(\s*function\s*\((require[,\s\w]*)?\)\s*\{/, replacer);
};
